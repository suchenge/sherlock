...
class Blueprint(object):
    def __init__(self, file: File, timer_index: int, timer):
        self.__output_name__ = '%s_%s.%s' % (file.title, str(timer_index + 1).zfill(5), file.type)
        self.__output_path__ = os.path.join(file.folder, self.__output_name__)
